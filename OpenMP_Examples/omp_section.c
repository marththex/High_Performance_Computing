...
#pragma omp parallel shared(a,b,c,d,nthreads) private(i,tid) //parallel directive
  {
  tid = omp_get_thread_num(); //get the tread #
  if (tid == 0)
    {
    nthreads = omp_get_num_threads(); //get # of threads
    printf("Number of threads = %d\n", nthreads);
    }
  printf("Thread %d starting...\n",tid);

  #pragma omp sections nowait // Does not wait for each section to finish
    {
